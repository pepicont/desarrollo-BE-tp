...
type TipoProducto = 'juego' | 'complemento' | 'servicio'

function getEntityByTipo(tipo: TipoProducto) {
  if (tipo === 'juego') return Juego
  if (tipo === 'complemento') return Complemento
  return Servicio
}

function sanitizeFotoInput(req: Request, _res: Response, next: NextFunction) {
...
async function create(req: Request, res: Response) {
  try {
    ...
    const productRef = em.getReference(Entity as any, id)

    const foto = new FotoProducto()
    foto.url = req.body.sanitizedInput.url
    foto.esPrincipal = !!req.body.sanitizedInput.esPrincipal
    if (tipo === 'juego') (foto as any).juego = productRef
    if (tipo === 'complemento') (foto as any).complemento = productRef
    if (tipo === 'servicio') (foto as any).servicio = productRef
...
async function setPrincipal(req: Request, res: Response) {
  try {
    const tipo = req.params.tipo as TipoProducto
    const id = Number(req.params.id)
    ...
    try {
      ...
      for (const f of todas) f.esPrincipal = f.id === fotoId
      await em.flush()
      await em.commit()
    } catch (e) {
      await em.rollback()
