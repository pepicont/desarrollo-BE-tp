...
async function add(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const detalle: string | undefined = req.body?.sanitizedInput?.detalle;
...
async function getByProduct(req: Request, res: Response): Promise<void> {
  try {
    ...
    let where: any;
    if (tipo === 'juego') where = { venta: { juego: id } };
...
async function getAllResenasAdmin(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const userId = req.user?.id;
...
async function removeAsAdmin(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const id = Number.parseInt(req.params.id);
    const userId = req.user?.id;
    const userTipo = req.user?.tipoUsuario;
